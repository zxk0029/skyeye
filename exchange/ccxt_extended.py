...
from exchange.controllers import set_ohlcv, set_market_data
...
class MarketDataCrawler:
    ...
    async def crawl_ohlcv_all_timeframes(self, symbol: Symbol):
        """抓取指定交易对的所有时间周期K线数据"""
        ...
        for timeframe in TIMEFRAMES.keys():
            try:
                await client.fetch_ohlcv(symbol.name, timeframe)
                self.logger.debug(f"Successfully fetched {timeframe} OHLCV for {symbol.name} from {symbol.exchange.name}")
            except Exception as e:
                self.logger.error(f"Failed to fetch {timeframe} OHLCV", exc_info=True)
                
    async def crawl_all_market_data(self, symbol: Symbol):
        """抓取指定交易对的综合市场数据"""
        client = self.clients.get(symbol.exchange.name)
        if not client:
            self.logger.warning(f"No client initialized for {symbol.exchange.name}")
        ...
        try:
            await client.fetch_market_data(symbol.name)
            self.logger.debug(f"Successfully fetched market data for {symbol.name} from {symbol.exchange.name}")
        except Exception as e:
            self.logger.error(f"Failed to fetch market data", exc_info=True)
    
    async def start_periodic_crawl(self, symbols: List[Symbol], interval: int = 60):
        """启动定期数据抓取
